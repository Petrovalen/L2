...
class ArduinoLink:
    ...
    def _send(self, cmd, wait_ok=True):
        ...
        with self._lock:
            try:
                ...
                return False
            if not wait_ok:
                # «Выстрелил-и-забыл»: НЕ ждём "OK", чтобы не блокировать игровой
                # цикл (иначе каждый нажим стопорит цикл на время удержания клавиши,
                # а при потерянном "OK" — до 1.5с таймаута). Изредка вычищаем
                # входной буфер от накопившихся подтверждений, чтобы он не рос.
                try:
    ...
    def key(self, name):
        # Клавиши — позиционно-независимы: шлём без ожидания "OK", чтобы нажатия
        # (атака/скиллы/подбор лута) не блокировали игровой цикл. Клики ниже
        # остаются синхронными (там важен порядок с установкой курсора).
        return self._send("KEY %s" % name, wait_ok=False)
